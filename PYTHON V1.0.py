...
class CobotApp(ctk.CTk):
    def __init__(self):
        ...
        # Botón ejecutar
        ctk.CTkButton(tray_scroll_frame, text="🚀 EJECUTAR TRAYECTORIA COORDINADA", 
                     fg_color="#388E3C", command=self.ejecutar_trayectoria, height=50).pack(pady=10)
    ...
    def update_trayectorias_info(self):
        ...
        if tray_name in self.trayectorias:
            ...
            self.tray_info_label.configure(text=info_text[:80] + "..." if len(info_text) > 80 else info_text)
        else:
    ...
    def cargar_trayectoria(self, nombre):
        """Cargar trayectoria seleccionada"""
        if nombre in self.trayectorias:
            tray = self.trayectorias[nombre]
            
            # Cargar posiciones (pueden ser negativas)
            for i, pos in enumerate(tray["pos"]):
