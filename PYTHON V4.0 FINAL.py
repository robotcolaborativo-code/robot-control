...
class CobotApp(ctk.CTk):
    ...
    def construir_comando_pickplace(self, pick_tray, place_tray, delay, garra_accion):
        ...
        # Construir comandos TRAY individuales
        pick_cmd = self.construir_comando_tray(pick_tray)
        place_cmd = self.construir_comando_tray(place_tray)
        
        # Usar velocidad de la primera trayectoria (o la mayor)
        velocidad = max(pick_tray.get("vel", 500), place_tray.get("vel", 500))
        
        # Construir comando PICKPLACE completo
        # Formato: PICKPLACE:TRAY:...:TRAY:...:vel:delay:garra_accion
        return f"PICKPLACE:{pick_cmd}:{place_cmd}:{velocidad}:{delay}:{garra_accion}"
    
    def construir_comando_tray(self, tray):
        """Construir comando TRAY desde diccionario"""
        targets = tray["pos"]
        direcciones = tray.get("dirs", "HHHH")
        velocidad = tray.get("vel", 500)
        garra = tray.get("garra", "ABRIR")
        
        return f"TRAY:{targets[0]},{targets[1]},{targets[2]},{targets[3]},{direcciones},{velocidad},{garra}"
    ...
    def ejecutar_secuencia_combinada_sincrona(self, tray):
        ...
        try:
            ...
            # Construir comando PICKPLACE
            delay = tray["delay"]
            garra_accion = tray["garra_accion"]
            
            comando_pickplace = self.construir_comando_pickplace(pick_tray, place_tray, delay, garra_accion)
            
            self.log(f"🤖 Ejecutando PICK-AND-PLACE: {pick_name} → {place_name}")
            self.log(f"  Pick: {pick_tray['pos']} {pick_tray.get('dirs', 'HHHH')}")
            self.log(f"  Place: {place_tray['pos']} {place_tray.get('dirs', 'HHHH')}")
            self.log(f"  Delay: {delay}s, Garra acción: {garra_accion}")
            
            # Enviar comando al ESP32
            if self.send_command(comando_pickplace, pause=1.0):
                self.log("✅ Comando PICKPLACE enviado correctamente")
                return True
            else:
                self.log("❌ Error al enviar comando PICKPLACE")
                return False
    ...
    def ejecutar_trayectoria_como_comando(self, tray):
        """Ejecutar una trayectoria desde su diccionario"""
        # Construir comando TRAY
        comando = self.construir_comando_tray(tray)
        
        if self.send_command(comando, pause=0.5):
            self.log(f"  ✅ Trayectoria ejecutada: {tray['pos']}")
            return True
        else:
            self.log("  ❌ Error al ejecutar trayectoria")
            return False
    ...
    def ejecutar_secuencia_combinada(self):
        """Ejecutar una secuencia pick-and-place completa"""
        if not self.connected:
            self.log("⚠️ No conectado al robot")
            return
        ...
        # Construir comando PICKPLACE
        comando_pickplace = self.construir_comando_pickplace(
            pick_tray, place_tray, tray["delay"], tray["garra_accion"]
        )
        
        # Ejecutar comando
        if self.send_command(comando_pickplace, pause=0.5):
            self.log("✅ Comando PICK-AND-PLACE enviado exitosamente")
        else:
            self.log("❌ Error al enviar comando PICK-AND-PLACE")
                    ...
                    while '\n' in buffer:
                        ...
                        if line:
                            ...
                            if "Cero referencia establecido" in line or "Cero de referencia establecido" in line:
                                ...
                                self.update_zero_display()
                                self.current_virtual_pos = [0.0, 0.0, 0.0, 0.0]
                            if "ESTABLECIENDO SISTEMA EN CERO" in line:
                                self.log("✅ Sistema establecido en cero para segunda trayectoria")
                                self.zero_set = True
                                self.current_virtual_pos = [0.0, 0.0, 0.0, 0.0]
                                self.update_zero_display()
            except Exception:
    ...
    def send_command(self, cmd, pause=None):
        ...
        if self.emergency_stop_active and cmd.upper() not in ["RESET", "ON", "STOP", "ESTOP", "E-STOP", "EMERGENCY", "ZERO", "SETZERO", "PICKPLACE:"]:
    ...
    def save_trayectoria(self):
        ...
        try:
            ...
            posiciones = []
            for entry in enumerate(self.tray_entries):
                i, entry_widget = entry
